...
The tldraw-era room class is still declared (`worker/LegacyTldrawDurableObject.ts`) so its data is not dropped. Admins can pull everything it holds from `/api/admin/legacy/tldraw` as JSON. When that content is no longer wanted, add a migration to `wrangler.jsonc`:
